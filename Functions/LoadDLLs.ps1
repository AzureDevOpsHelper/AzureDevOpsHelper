...
function Import-GraphAssemblies
{
    #Write-Host "Checking and loading the MSAL.NET assemblies."
    $WP = $WarningPreference
    $WarningPreference = 'SilentlyContinue'
    ...
    try 
    {
        $AIdll = Resolve-Path ".\Libraries\azure.identity\1.13.2\lib\net8.0\Azure.Identity.dll"
        Add-Type -Path $AIdll
        [Reflection.Assembly]::Load("Azure.Identity") | Out-Null
    }
    catch 
    {
        if ($_ -match "Assembly with same name is already loaded")
        {
            Write-Host "Azure.Identity.dll is already loaded."
        }
        else
        {
            Write-Host $_
            Write-Host "Error loading Azure.Identity.dll you can try to download it from nuget and place it in the Libraries folder."
        }
    }
    try
    {
        $MGdll = Resolve-Path ".\Libraries\microsoft.graph\5.72.0\lib\net5.0\Microsoft.Graph.dll"
        Add-Type -Path $MGdll 
        [Reflection.Assembly]::Load("Microsoft.Graph") | Out-Null
    }
    catch 
   {
        if ($_ -match "Assembly with same name is already loaded")
       {
            Write-Host "Microsoft.Graph.dll is already loaded."
        }
        else
        {
            Write-Host $_
            Write-Host "Error loading Microsoft.Graph.dll you can try to download it from nuget and place it in the Libraries folder."
        }
    }
    $WarningPreference = $WP
    $ProgressPreference = $PP
